...
-- Source: ~/.config/hypr/core/* (glob; resolved at runtime). Each matched .conf must be converted to .lua.
hl_source_glob("~/.config/hypr/core/*")
-- Source: ~/.config/hypr/extra/rules.conf — convert this file to Lua and ensure it is on Lua's package.path.
require("extra.rules")
-- Source: ~/.config/hypr/extra/plugins/* (glob; resolved at runtime). Each matched .conf must be converted to .lua.
hl_source_glob("~/.config/hypr/extra/plugins/*")

-- opengl:nvidia_anti_flicker = false # reduces flickering on nvidia at the cost of possible frame drops on lower-end GPUs. On non-nvidia, this is ignored.
