...
# ------------ YAY ------------
yayinstall () { yay -Slq | fzf -q "$1" --multi --preview 'yay -Si {1} | bat -n --color=always -l yaml' | xargs echo >/dev/stderr | xargs -ro yay -S }
yayremove () { yay -Qq | fzf -q "$1" --multi --preview 'yay -Qi {1} | bat -n --color=always -l yaml' | xargs echo >/dev/stderr | xargs -ro yay -Rns }
